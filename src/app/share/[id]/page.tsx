...
export default function SharePage() {
  ...
  useEffect(() => {
    const fetchSharedData = async () => {
      try {
        const response = await fetch(`/api/cache/get/${params.id}`)
        const result = await response.json()
        
        if (result.success && result.data) {
          setCurrentPipeline(result.data.pipeline || result.data)
        } else {
          setError(result.error || 'Failed to load shared analysis')
        }
      } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-black text-gray-100 flex items-center justify-center">
        <div className="text-center">
          <div className="text-2xl font-mono text-orange-700 mb-4 animate-pulse">STARTING_ANALYSIS_ENGINE</div>
        </div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-black text-gray-100 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-mono text-red-600 mb-4">ERROR_LOADING_DATA</h2>
  ...
  return (
    <div className="min-h-screen bg-black text-gray-100">
      ...
      <div className="bg-zinc-900 border-b border-zinc-800">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm sm:text-lg font-mono text-orange-700">
                OFFICE_POOL_ANALYSIS_ENGINE
              </h2>
      ...
      <div className="max-w-7xl mx-auto px-4 py-2 sm:py-6">
        ...
        {currentPipeline?.comparison?.comparisons && (
          <div className="bg-zinc-900 rounded border border-zinc-800 p-4 mb-4 relative">
            ...
            <div className={`${showFilters ? 'grid grid-cols-2 gap-3' : 'hidden'} sm:flex sm:flex-wrap sm:items-end sm:gap-2 mb-1 sm:mb-2`}>
              {/* Clear Filters Button - moved to left */}
              <button
                onClick={() => {
                  setFilters({
                    league: 'all',
        ...
        {/* Week Display and Guide Button - Between filter and table */}
        {currentPipeline?.comparison?.comparisons && (
