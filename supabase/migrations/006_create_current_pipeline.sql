...
CREATE TABLE IF NOT EXISTS current_pipeline (
  ...
  -- Alternative: use a singleton pattern with a fixed ID
  is_current BOOLEAN DEFAULT TRUE
);

-- Create index on is_current for faster lookups
CREATE INDEX IF NOT EXISTS idx_current_pipeline_is_current ON current_pipeline(is_current) WHERE is_current = true;

-- Create index on updated_at for sorting
CREATE INDEX IF NOT EXISTS idx_current_pipeline_updated_at ON current_pipeline(updated_at DESC);
