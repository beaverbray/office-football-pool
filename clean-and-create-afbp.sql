...
-- Drop the afbp schema completely (if it exists)
...
CREATE TABLE afbp.core_schedule (
...
CREATE TABLE afbp.analysis_predictions (
...
-- 4. Shared Analyses
CREATE TABLE afbp.shared_analyses (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE afbp.pipeline_job_runs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE afbp.pipeline_picks_rows (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_predictions_source ON afbp.analysis_predictions(source);
CREATE INDEX idx_predictions_scraped_at ON afbp.analysis_predictions(scraped_at DESC);
CREATE INDEX idx_shared_analyses_share_id ON afbp.shared_analyses(share_id);
CREATE INDEX idx_shared_analyses_expires_at ON afbp.shared_analyses(expires_at);
